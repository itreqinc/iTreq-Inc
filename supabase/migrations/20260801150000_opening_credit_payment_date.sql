-- Adjustment payments from brought-forward credit must keep the original
-- opening-balance date, not the day the credit was applied to an invoice.
...
begin
  ...
  pay_date := coalesce(cl.opening_balance_date, current_date);
...
-- Fix any adjustment rows already written with "applied today" dates.
update public.payments p
set payment_date = c.opening_balance_date
from public.clients c
where p.client_id = c.id
  and p.is_adjustment = true
  and p.notes = 'Applied from brought-forward credit'
  and c.opening_balance_date is not null
  and p.payment_date is distinct from c.opening_balance_date;

-- Phase 4: monthly fee invoices by copying fee lines from previous month.

alter table public.invoices
  add column if not exists billing_period date;

comment on column public.invoices.billing_period is
  'First day of the month this invoice covers for recurring monitoring fees; null for install/one-off invoices.';

create unique index if not exists invoices_client_billing_period_uidx
  on public.invoices (client_id, billing_period)
  where billing_period is not null and status <> 'void';
...
/**
 * Prefer invoice with billing_period = previous month;
 * else latest invoice in previous calendar month that has fee lines (tracks_stock = false).
 */
...
begin
  ...
  loop
    ...
    if v_source is null then
      -- Only report clients who might be relevant: skip silent "no source" for everyone
      -- Preview focuses on would-create + already-billed + no-source only if they had any prior invoice
      if exists (
        select 1 from public.invoices i
...
grant execute on function public.month_start(date) to anon, authenticated;
grant execute on function public.find_monthly_fee_source_invoice(uuid, date) to anon, authenticated;
grant execute on function public.preview_monthly_fee_invoices(date) to anon, authenticated;
grant execute on function public.generate_monthly_fee_invoices(date) to anon, authenticated;

-- When original B/F is later reduced below an existing payment's B/F share,
-- allow that payment to keep (or reduce) its allocation. Increasing it still
-- cannot exceed remaining excluding this payment.
...
declare
  ...
  old_applied numeric(12, 2);
  allowed_bf numeric(12, 2);
...
begin
  ...
  old_applied := round(greatest(0, -coalesce(pay.opening_balance_delta, 0)), 2);
  allowed_bf := round(greatest(remaining_bf, old_applied), 2);

  if opening_amt > 0 then
    if allowed_bf <= 0 then
      raise exception 'This client has no positive brought-forward balance to pay';
    end if;
    if opening_amt > allowed_bf + 0.001 then
      raise exception

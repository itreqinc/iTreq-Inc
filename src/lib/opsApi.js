...
const directOpsApi = {
  ...
  async deleteInvoice(id) {
    ...
    const { error: byConvertedErr } = await supabase
      .from('quotations')
      .update({
        status: 'draft',
      ...
      .eq('status', 'converted')
    if (byConvertedErr) return mapError(byConvertedErr)

    if (existing.data.quotation_id) {
      const { error: byLinkErr } = await supabase
        .from('quotations')
        .update({
          status: 'draft',
          converted_invoice_id: null,
        ...
        .eq('id', existing.data.quotation_id)
        .eq('status', 'converted')
      if (byLinkErr) return mapError(byLinkErr)
    }

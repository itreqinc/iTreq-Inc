 ...
 * }} input
 */
export function buildExpenseCategoryReportModel({
  report,
  settings,
  ...
  const source = report?.categoryTotals || report?.byCategory || []
  const categories = (includeZeroCategories ? source : source.filter((row) => Number(row.amount) > 0)).map(
    (row) => ({
  ...
  return {
    ...
    includeZeroCategories: Boolean(includeZeroCategories),
    currency,
    total,
...
export function getExpenseCategoryReportPrintPath(model) {
  const period = [model.from, model.to].filter(Boolean).join('_') || 'period'
  return `/admin/print/expenses-by-category/${period}`
}
...
const PRINT_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .doc-meta {
    ...
    max-width: 16rem;
    text-align: right;
    /* Sit the title midline on the logo midline (80px slot), keep line spacing below. */
...
function renderBody(model) {
  ...
  const email = model.company.email
    ...
    : ''
  const filterNote = model.includeZeroCategories
    ? 'Includes all expense categories (including those with no spend in this period).'
    : 'Includes only categories with spend greater than zero in this period.'

  const rows =
    model.categories.length === 0
      ? `<tr><td colspan="2">No categories to show for this period.</td></tr>`
      : model.categories
          .map(
            (row) => `<tr>
        ...
        <div class="head">
          <div class="head-top">
            ...
            <div class="doc-meta">
              ...
              <p><strong>Expenses recorded:</strong> ${escapeHtml(String(model.expenseCount))}</p>
            </div>
          </div>
...
export function buildExpenseCategoryReportPrintPage(model) {
